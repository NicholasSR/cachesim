...
public class Cache {
    ...
    public Cache(int capacity, String filePath) {
        ...
        backingStore = new BackingStore(filePath);
    }
    ...
    public CacheResponse writeData(int key, int newData) throws NotFoundException {
        ...
        if (idx < 0) {
            BackingStoreResponse bsr = backingStore.fetchData(key);
            time += bsr.getResponseTime();
            int data = bsr.getData();
        ...
        updateRanks(idx);
        CacheResponse ret = new CacheResponse(this.data[idx].copy(), miss, time);
        return ret;
    }
    ...
    public ArrayList<CacheItem> getContents() {
        ArrayList<CacheItem> contents = new ArrayList<>(this.capacity);
        for (int i = 0; i < this.capacity; i++) {
            if (data[i] != null) {
